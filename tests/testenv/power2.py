...
def relaystate():
    ...
    if len(answer) < 4:
        print("\t\t\tNO ANSWER!!!")
        return 0
    tmp = (answer[3] << 8) + answer[4]
    print("relays state: ", end="")
    for i in range(0, 16):
        if i == 15:
            end = "\n"
        else:
...
def relayon(num):
    print("turn", num, "/ 15 relay on")
    tmp = 1 << num
    cmd = [ 0x01,       #device addr
            0x10,       #operation code, write regs
            0x00, 0x00, #reg start addr
            0x00, 0x02, #regs num
            0x04,       #digital length?
            0x00, 0x01, #data
...
parser.add_argument("mode", help="What to do with power (on|off|reset|alloff)")
parser.add_argument("--num", type=int, help="Target device's number")

args = parser.parse_args()
if args.mode not in ("on", "off", "reset", "alloff"):
    print("Allowed modes: 'on', 'off', 'reset' or 'alloff'")
    exit(1)

if args.mode == "alloff":
    print("RELAY1:")
    relay = serial.Serial(PORT_RELAY1, BAUDRATE_RELAY1, timeout = TIMEOUT)
    relaystate(); relayalloff(); relaystate()
    relay.close()
    print("RELAY2:")
    relay = serial.Serial(PORT_RELAY2, BAUDRATE_RELAY2, timeout = TIMEOUT)
    relaystate(); relayalloff(); relaystate()
    relay.close()
    exit(0)

if args.num < 1 or args.num > 32:
...
if args.mode == "reset":
    pre_state = relaystate()
    relayon(DEVICE)
    new_state = relaystate()
    expected_state = pre_state | (1 << DEVICE)
